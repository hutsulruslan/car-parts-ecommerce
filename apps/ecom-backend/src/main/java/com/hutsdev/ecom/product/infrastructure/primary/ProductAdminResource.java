...
import com.hutsdev.ecom.product.infrastructure.primary.exceptions.EntityCreationFailed;
import com.hutsdev.ecom.product.infrastructure.primary.exceptions.MultipartPictureException;
...
public class ProductAdminResource {
  ...
  private static final Logger log = LoggerFactory.getLogger(ProductAdminResource.class);
  ...
  public ResponseEntity<UUID> delete(@RequestParam("publicId") UUID id) {
    try {
      ...
      log.error("Could not delete product with id {}", id, enfee);

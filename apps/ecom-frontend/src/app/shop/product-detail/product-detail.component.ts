...
export class ProductDetailComponent {
  ...
  private handleRelatedProductQueryError() {
    if (this.relatedProductQuery.isError()) {
      this.toastService.show('Помилка! Не вдалося завантажити пов’язані товари. Будь ласка, спробуйте ще раз.', 'ERROR');
    }
  }

  private handleProductQueryError() {
    if (this.productQuery.isError()) {
      this.toastService.show('Помилка! Не вдалося завантажити товар. Будь ласка, спробуйте ще раз.', 'ERROR');
    }
  }

  addToCart(productToAdd: Product) {
    this.cartService.addToCart(productToAdd.publicId, 'add');
    this.labelAddToCart = 'Додано';
    this.iconAddToCart = 'check';

    interval(3000)
      .pipe(take(1))
      .subscribe(() => {
        this.labelAddToCart = 'Додати в кошик';
